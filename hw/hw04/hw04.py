...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    left_thing = end(left(m))
    right_thing = end(right(m))
    if is_mobile(left_thing):
        if not balanced(left_thing):
            return False 
    if is_mobile(right_thing):
        if not balanced(right_thing):
            return False

    left_torque = length(left(m)) * total_mass(left_thing)
    right_torque = length(right(m)) * total_mass(right_thing)
    print("DEBUG: ", left_torque, right_torque)
