...
class FreeChecking(Account):
    ...
    def __init__(self, account_holder):
        ...
        self.remain_free_withdrawals = self.free_withdrawals


    def withdraw(self, amount):
        fee = 0 if self.remain_free_withdrawals > 0 else self.withdraw_fee
        if self.remain_free_withdrawals > 0:
            self.remain_free_withdrawals -= 1
        amount += fee
        if amount > self.balance:
            return "Insufficient funds"
        if amount > self.max_withdrawal:
...
def without(s, i):
    ...
    #     if i != 0:
    #         cur_ans.rest = Link(cur.first)
    #         cur_ans = cur_ans.rest
    #     i -= 1
    #     cur = cur.rest
    # return ans.rest
