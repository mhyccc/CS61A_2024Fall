...
def distance(city_a, city_b):
    ...
    return sqrt(pow(city_a_lat - city_b_lat, 2) + pow(city_a_lon - city_b_lon, 2))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    dis_a = distance(city_a, make_city('', lat, lon))
    dis_b = distance(city_b, make_city('', lat, lon))

    # if dis_b <= dis_a:
    #     return get_name(city_b)
    # return get_name(city_a)

    return get_name(city_a) if dis_b > dis_a else get_name(city_b)
